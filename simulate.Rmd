...
These are the values we'll use in the simulation. With the square root of the sum of all the variance components equal to `r round(denom, 2)`, a Cohen's d-like value of 0.30 would be a raw difference of `0.65*0.3 = 0.195`.
...
> The standard deviation of the group random intercept and leader random intercept is estimated to be zero, which is in line with what you simulated. These random intercepts are only defined in the treatment arm (which you can tell from the output since it says "treatment" under Name.

> Technically, this is parameterized as a random slope, but we are suppressing the separate random intercept. Normally, we would think of a slope as being defined on a continuous value (e.g., continuous time). Since treatment is a binary variable and we suppress the random intercept, it is essentially a random intercept in the treatment arm only.

> The standard deviation of the family random intercept is 0.398, which is close to what you simulated based on. This random intercept is defined in both treatment arms, since we expect there to be clustering within family regardless of treatment arm.
...
Small effects (0.3) will be difficult to detect with anything less than 400 families if the pilot data are any guide. 

```{r, echo=FALSE, dpi = 300}
effect_type <- "d_t"
effect_y <- ifelse(effect_type=="d_t", "Estimate (d_t)", "Estimate (raw)")
x <- r1
...
# !! must group by all crossings that are not fixed
  d_mean <- x %>% 
    filter(term == "treatment", type == effect_type) %>% 
    group_by(families, b1) %>%
    summarise(d_mean = mean(estimate)) 
  ...
  x %>% 
    ...
    mutate(below = ifelse(conf.low < 0, "yes", "no")) %>%
    ...
    labs(x = "Simulation index",
         y = effect_y,
         title = "Power simulation (lme4)",
         subtitle = str_wrap(paste0(sims, " simulations each panel. Point estimates and ", min(x$ci)*100, "% intervals. Simulation inputs informed by point estimates of small pilot."), 100),
...
Next we take the same NHST approach to power from a Bayesian perspective. We fit the model once with a strongly informative prior on treatment and then use `update()` to fit without recompiling in the simulation.
...
# !! must group by all crossings that are not fixed
  ...
  x %>% 
    ...
    labs(x = "Simulation index",
         y = effect_y,
         title = "Power simulation (brms, N(0.3, 0.2) prior on treatment)",
         subtitle = str_wrap(paste0(sims, " simulations each panel. Point estimates (mean) and ", min(x$ci)*100, "% intervals. Simulation inputs informed by point estimates of small pilot."), 100),
         caption = str_wrap(paste0("Data simulated with the following random intercepts (SD): ", u0l_sd, " leader; ", u0g_sd, " group; ", u0f_sd, " family; ", sigma_sd, " residual error. N(0.3, 0,2) prior on effect."), 120)
    )
```
...
FYI: We'd have greater power to detect effects if we assume the lower bound on the random effects from the previous small pilot.
...
Talk about how to analyze data from multiple informants. The current setup uses data from all family members as if we're estimating the average impact on a person's perceived functioning of their family. But we can also think of family functioning of something that exists at the level of the family. [Kenny has some notes](https://academic.oup.com/jpepsy/article/36/5/630/924908) about approaches.
